...
Built on [Meta Wearables DAT SDK](https://github.com/facebook/meta-wearables-dat-ios) (iOS) / [DAT Android SDK](https://github.com/nichochar/openclaw) (Android) + [Volcengine Doubao Realtime API](https://www.volcengine.com/product/doubao) + [OpenClaw](https://github.com/nichochar/openclaw) (optional).
...
- **"What am I looking at?"** -- Doubao sees through your glasses camera and describes the scene (Note: Video streaming natively over Doubao WebSockets is pending)
...
The glasses camera flows to the application for visual context, while audio streams bidirectionally in real-time to Doubao.
...
Doubao Realtime API (WebSocket)
       ...
       |
       v
  Doubao speaks the result
```

**Key pieces:**
- **Doubao Realtime** -- real-time voice AI over WebSocket (native audio, not STT-first)
- **OpenClaw** (optional) -- local gateway that gives Doubao access to 56+ tools and all your connected apps
...
Edit `Secrets.swift` with your Doubao API credentials (App ID, Access Key, App Key, Resource ID) and optional OpenClaw/WebRTC config.
...
2. Tap the **AI button** to start a Doubao Live session
3. Talk to the AI
...
Edit `Secrets.kt` with your Doubao API credentials (App ID, Access Key, App Key, Resource ID) and optional OpenClaw/WebRTC config.
...
2. Tap the **AI button** (sparkle icon) to start a Doubao Live session
3. Talk to the AI
...
OpenClaw gives Doubao the ability to take real-world actions: send messages, search the web, manage lists, control smart home devices, and more. Without it, Doubao is voice only (vision pending).
...
| `Gemini/GeminiLiveService.swift` | WebSocket client for Doubao Realtime API |
...
| `OpenClaw/ToolCallRouter.swift` | Routes Doubao tool calls to OpenClaw |
...
| `gemini/GeminiLiveService.kt` | OkHttp WebSocket client for Doubao Realtime API |
...
| `openclaw/ToolCallRouter.kt` | Routes Doubao tool calls to OpenClaw |
...
- **Input**: Phone mic -> AudioManager (PCM Int16, 16kHz mono, 100ms chunks) -> Doubao WebSocket
- **Output**: Doubao WebSocket -> AudioManager playback queue -> Phone speaker
...
- **Glasses**: DAT SDK video stream (24fps) -> throttle to ~1fps -> JPEG (50% quality) -> Doubao (pending)
- **Phone**: Camera capture (30fps) -> throttle to ~1fps -> JPEG -> Doubao (pending)

### Tool Calling

Doubao supports function calling. Both apps declare a single `execute` tool that routes everything through OpenClaw:

1. User says "Add eggs to my shopping list"
2. Doubao speaks "Sure, adding that now" (verbal acknowledgment before tool call)
3. Doubao sends `toolCall` with `execute(task: "Add eggs to the shopping list")`
4. `ToolCallRouter` sends HTTP POST to OpenClaw gateway
5. OpenClaw executes the task using its 56+ connected skills
6. Result returns to Doubao via `toolResponse`
7. Doubao speaks the confirmation
...
**Doubao doesn't hear me** -- Check that microphone permission is granted. The app uses aggressive voice activity detection -- speak clearly and at normal volume.
...
**"API key not configured"** -- Add your credentials in Secrets.swift or in the in-app Settings.
...
**Doubao WebSocket times out** -- The Doubao Realtime API sends binary WebSocket frames. If you're building a custom client, make sure to handle both text and binary frame types according to the 4-byte header spec.
